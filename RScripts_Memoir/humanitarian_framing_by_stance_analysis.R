# R Script: Humanitarian Framing During Heightened Violence by Stance
# Model: is_humanitarian_frame ~ Time + heightened_violence * time
# Run separately for Pro-Palestinian, Pro-Israeli, and All Posts
...
cat("=== HUMANITARIAN FRAMING DURING HEIGHTENED VIOLENCE BY STANCE ===\n")
cat("Model: is_humanitarian_frame ~ months_since_start + heightened_violence * months_since_start\n\n")
...
# ============================================================================
# REGRESSION: Humanitarian Framing ~ Time + Heightened Violence * Time
# ============================================================================

cat("=== REGRESSION MODEL ===\n")
cat("is_humanitarian_frame ~ months_since_start + heightened_violence * months_since_start\n\n")

# Model for Pro-Palestinian posts
cat("--- Pro-Palestinian Posts ---\n")
model_pro_pal <- glm(is_humanitarian_frame ~ months_since_start + 
...
model_pro_isr <- glm(is_humanitarian_frame ~ months_since_start + 
                     heightened_violence * months_since_start, 
                     data = data_pro_isr, 
                     family = binomial(link = "logit"))
...
cat("--- Neutral Posts ---\n")
model_neutral <- glm(is_humanitarian_frame ~ months_since_start + 
                     heightened_violence * months_since_start, 
                     data = data_neutral, 
                     family = binomial(link = "logit"))
...
model_all <- glm(is_humanitarian_frame ~ months_since_start + 
                 heightened_violence * months_since_start, 
                 data = data_all, 
                 family = binomial(link = "logit"))
...
p1 <- ggplot(p1_data, aes(x = date, y = prob, color = period, linetype = period)) +
  ...
  labs(
    title = "Predicted Probability of Humanitarian Framing by Stance",
    subtitle = "Model: Time + Heightened Violence * Time",
    x = "Month",
    y = "Predicted Probability of Humanitarian Framing",
    color = "Period",
    linetype = "Period"
  ) +
  theme_minimal() +
...
ggsave("humanitarian_framing_by_stance.png", plot = p1, width = 18, height = 5, dpi = 300)
cat("Plot saved to: humanitarian_framing_by_stance.png\n")
...
p2 <- ggplot(coef_data, aes(x = Coefficient, y = Value, fill = Stance)) +
  ...
  scale_fill_manual(values = c("Pro-Palestinian" = "forestgreen", 
                               ...
                               "All Posts" = "gray50")) +
  labs(
    title = "Coefficients Comparison by Stance",
    subtitle = "Logistic Regression: Humanitarian Framing ~ Time + Heightened Violence * Time",
    x = "Coefficient",
    y = "Coefficient Value",
    fill = "Stance"
  ) +
...
ggsave("humanitarian_framing_coefficients_comparison.png", plot = p2, width = 12, height = 6, dpi = 300)
cat("Plot saved to: humanitarian_framing_coefficients_comparison.png\n")
...
p3 <- ggplot(or_data, aes(x = Coefficient, y = Odds_Ratio, fill = Stance)) +
  ...
  scale_fill_manual(values = c("Pro-Palestinian" = "forestgreen", 
                               ...
                               "All Posts" = "gray50")) +
  labs(
    title = "Odds Ratios Comparison by Stance",
...
ggsave("humanitarian_framing_odds_ratios_comparison.png", plot = p3, width = 12, height = 6, dpi = 300)
cat("Plot saved to: humanitarian_framing_odds_ratios_comparison.png\n")
...
for (i in 1:nrow(comparison_table)) {
  ...
  if (comparison_table$Interaction_P[i] < 0.05) {
    cat("  → The effect of violence on humanitarian framing ", 
