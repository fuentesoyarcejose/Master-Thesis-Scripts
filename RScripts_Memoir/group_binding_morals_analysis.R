# R Script: Group-Binding Morals (Loyalty + Authority) During Heightened Violence
# Model: is_group_binding_frame ~ Time + heightened_violence * time
# Combines Loyalty and Authority into one analysis
...
cat("=== GROUP-BINDING MORALS (LOYALTY + AUTHORITY) DURING HEIGHTENED VIOLENCE ===\n")
cat("Model: is_group_binding_frame ~ months_since_start + heightened_violence * months_since_start\n\n")
...
# Define group-binding framing (Loyalty OR Authority)
data$is_group_binding_frame <- ifelse(data$`highest score` %in% c("Loyalty", "Authority"), 1, 0)
...
cat("Group-binding framing (Loyalty or Authority) prevalence:\n")
cat("  - Pro-Palestinian:", round(mean(data_pro_pal$is_group_binding_frame) * 100, 2), "%\n")
cat("  - Pro-Israeli:", round(mean(data_pro_isr$is_group_binding_frame) * 100, 2), "%\n")
cat("  - Neutral:", round(mean(data_neutral$is_group_binding_frame) * 100, 2), "%\n")
cat("  - All posts:", round(mean(data_all$is_group_binding_frame) * 100, 2), "%\n\n")

# ============================================================================
# REGRESSION: Group-Binding Framing ~ Time + Heightened Violence * Time
# ============================================================================

cat("=== REGRESSION MODEL ===\n")
cat("is_group_binding_frame ~ months_since_start + heightened_violence * months_since_start\n\n")

# Model for Pro-Palestinian posts
cat("--- Pro-Palestinian Posts ---\n")
model_pro_pal <- glm(is_group_binding_frame ~ months_since_start + 
...
model_pro_isr <- glm(is_group_binding_frame ~ months_since_start + 
                     heightened_violence * months_since_start, 
                     data = data_pro_isr, 
                     family = binomial(link = "logit"))
...
cat("--- Neutral Posts ---\n")
model_neutral <- glm(is_group_binding_frame ~ months_since_start + 
                     heightened_violence * months_since_start, 
                     data = data_neutral, 
                     family = binomial(link = "logit"))
...
model_all <- glm(is_group_binding_frame ~ months_since_start + 
                 heightened_violence * months_since_start, 
                 data = data_all, 
                 family = binomial(link = "logit"))
...
p1 <- ggplot(p1_data, aes(x = date, y = prob, color = period, linetype = period)) +
  ...
  labs(
    title = "Predicted Probability of Group-Binding Framing (Loyalty + Authority) by Stance",
    subtitle = "Model: Time + Heightened Violence * Time",
    x = "Month",
    y = "Predicted Probability of Group-Binding Framing",
    color = "Period",
    linetype = "Period"
  ) +
  theme_minimal() +
...
ggsave("group_binding_framing_by_stance.png", plot = p1, width = 18, height = 5, dpi = 300)
cat("Plot saved to: group_binding_framing_by_stance.png\n")
...
p2 <- ggplot(coef_data, aes(x = Coefficient, y = Value, fill = Stance)) +
  ...
  scale_fill_manual(values = c("Pro-Palestinian" = "forestgreen", 
                               ...
                               "All Posts" = "gray50")) +
  labs(
    title = "Coefficients Comparison by Stance - Group-Binding Framing (Loyalty + Authority)",
    subtitle = "Logistic Regression: Group-Binding Framing ~ Time + Heightened Violence * Time",
    x = "Coefficient",
    y = "Coefficient Value",
    fill = "Stance"
  ) +
...
ggsave("group_binding_framing_coefficients_comparison.png", plot = p2, width = 12, height = 6, dpi = 300)
cat("Plot saved to: group_binding_framing_coefficients_comparison.png\n")
...
p3 <- ggplot(or_data, aes(x = Coefficient, y = Odds_Ratio, fill = Stance)) +
  ...
  scale_fill_manual(values = c("Pro-Palestinian" = "forestgreen", 
                               ...
                               "All Posts" = "gray50")) +
  labs(
    title = "Odds Ratios Comparison by Stance - Group-Binding Framing (Loyalty + Authority)",
...
ggsave("group_binding_framing_odds_ratios_comparison.png", plot = p3, width = 12, height = 6, dpi = 300)
cat("Plot saved to: group_binding_framing_odds_ratios_comparison.png\n")
...
cat("\n\n=== SUMMARY ===\n\n")
cat("Model: is_group_binding_frame ~ months_since_start + heightened_violence * months_since_start\n")
cat("Group-Binding Framing = Loyalty OR Authority\n\n")
...
for (i in 1:nrow(comparison_table)) {
  ...
  if (comparison_table$Interaction_P[i] < 0.05) {
    cat("  → The effect of violence on group-binding framing ", 
